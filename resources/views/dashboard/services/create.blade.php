    ...
    <div class="container">
        ...
        <form action="{{ isset($service) ? route('services.update', $service->id) : route('services.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="file" class="form-control" id="icon" name="icon" value="{{ old('icon', $service->icon ?? '') }}">
                @error('icon')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                @enderror
            </div>
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    <label for="description_ar" class="form-label">Service Description En</label>
                    <textarea class="form-control" id="description_ar" name="description[en]" rows="3">{{ old('description.ar', $service->description['ar'] ?? '') }}</textarea>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    <label for="short_description_ar" class="form-label">Service  Description Ar</label>
                    <textarea class="form-control" id="short_description_ar" name="description[ar]" rows="3">{{ old('short_description.ar', $service->short_description['ar'] ?? '') }}</textarea>
